...
class Object:
	...
	def x(self, x):
		...
		try:
			...
			raise TypeError("x must be an floating number")
	...
	def y(self, y):
		...
		try:
			...
			raise TypeError("y must be an floating number")
	...
	def mass(self, mass):
		...
		try:
			...
			raise TypeError("mass must be an floating number")
	...
	def gravitationStrenght(ma, mb, d):
		...
		"""
		return 6.67408e-11 * float(ma) * float(mb) / float(d) ** 2
	...
	def applyStrenght(*objects):
		"""calcule l'accélération de l'objet a partir des forces et des autres objects passé en paramètre

		Args:
			*objects: liste des objects ou des forces a apliquer

		Raises:
			TypeError: objects doit contenir uniquement des Vector, des Object ou des objects qui hérient de Object ou de Vector
		"""
		fSumm = Vector()
		for obj in objects:
			if isinstance(obj, Object):
				fSumm += this.getGravitationStrenght(obj)
